...
export class TrieNode {
  ...
  isVazio(raiz) {
    ...
    for (let i = 0; i < TAMANHO_ALFABETO; i++)
      if (raiz.filhos[i] != null) return false;
    return true;
  }
   ...
   * a ser removido.
   */
  remover(raiz, chave, profundidade) {
    ...
    if (profundidade == chave.length) {
      ...
      if (raiz.fimDaPalavra) raiz.fimDaPalavra = false;

      // Se a chave informada não for prefixo de alguma outra palavra
      if (this.isVazio(raiz)) {
        raiz = null;
      }
    ...
    raiz.filhos[index] = this.remover(raiz.filhos[index], chave, profundidade + 1);

    //Caso a raiz nao tenha nenhum filho (seu unico filho foi
    //deletado), e não é o fim de outra palavra.

    if (this.isVazio(raiz) && raiz.fimDaPalavra == false) {
      raiz = null;
    }
